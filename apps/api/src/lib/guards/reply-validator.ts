...
const PRICE_TEXT_PATTERN =
  /\b(?:R\$\s?\d+|\d{2,6}\s*(?:reais|real))\b/iu;
const CPF_PATTERN =
  /\b\d{3}\.?\d{3}\.?\d{3}-?\d{2}\b/;
...
const extractPrices = (text: string): number[] => {
  const matches = text.matchAll(/(?:R\$\s?)?(\d{1,3}(?:\.\d{3})*|\d+)(?:,(\d{2}))?\s*(?:reais|real)?/gi);
  const prices: number[] = [];
  for (const m of matches) {
    const full = m[0];
    if (!/R\$|reais|real/i.test(full)) continue;
...
export const validateReply = (text: string, context: ValidationContext): ValidationResult => {
  ...
  if (knownPrices.length === 0 && PRICE_TEXT_PATTERN.test(text)) {
    return { ok: false, reason: 'price_without_kb' };
  }
  if (knownPrices.length > 0) {
    for (const p of prices) {
      if (!knownPrices.includes(p)) return { ok: false, reason: `price_not_in_kb: ${p}` };
    }
  }

  if (CPF_PATTERN.test(text)) {
    return { ok: false, reason: 'pii_cpf' };
  }
